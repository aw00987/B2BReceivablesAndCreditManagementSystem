        ...
        <div>
            ...
            >
                <Form form={form} layout="vertical" onFinish={handleCreate}>
                    <Form.Item name="companyCode" label="取引先名"
                               rules={[{required: true, message: '取引先名を選択してください'}]}>
                        <Select
                            showSearch
                    ...
                    <Form.Item name="invoiceAmount" label="金额"
                               rules={[{required: true, message: '金额を入力してください'}]}>
                        <InputNumber style={{width: '100%'}}
                                     formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                     parser={value => value!.replace(/\¥\s?|(,*)/g, '')}/>
                    </Form.Item>
                    <Form.Item name="issueDate" label="発行日"
                               rules={[{required: true, message: '発行日を選択してください'}]}>
                        <DatePicker style={{width: '100%'}}/>
                    </Form.Item>
                    <Form.Item name="dueDate" label="支払期日"
                               rules={[{required: true, message: '支払期日を選択してください'}]}>
                        <DatePicker style={{width: '100%'}}/>
                    </Form.Item>
                ...
                {isDetailLoading ? (
                    ...
                    <div>
                        ...
                        <Descriptions title="金额详细" bordered column={2}>
                            <Descriptions.Item
                                label="請求金额">¥{invoice.invoiceAmount?.toLocaleString()}</Descriptions.Item>
                            <Descriptions.Item
                                label="元金">¥{invoice.principalAmount?.toLocaleString()}</Descriptions.Item>
                        ...
                        <div className="mt-4">
                            ...
                            {(invoice.status === '延滞' || invoice.status === '督促中') ? (
                                ...
                                <div
                                    className="p-3 border rounded bg-gray-50 h-32 overflow-y-auto whitespace-pre-wrap text-gray-600">
                                    {invoice.notes || '-'}
                                </div>
                            )}
                        </div>
